...
int rkqs_added(Project* project, double* x, int n, double htry, double eps, double* hdid,
...
void rkck_added(Project* project, double x, int n, double h, void(*derivs)(Project* project, double, double*, double*));
...
int odesolve_integrate_added(Project* project, double ystart[], int n, double x1, double x2,
	...
	//---------------------------------------------------------------
{
	...
	for (nstp = 1; nstp <= MAXSTP; nstp++)
	{
		...
		for (i = 0; i<n; i++)
			yscal[i] = fabs(y[i]) + fabs(dydx[i] * h) + TINY;
		if ((x + h - x2)*(x + h - x1) > 0.0) h = x2 - x;
		errcode = rkqs_added(project,&x, n, h, eps, &hdid, &hnext, derivs);
		if (errcode) break;
		if ((x - x2)*(x2 - x1) >= 0.0)
		{
			for (i = 0; i<n; i++) ystart[i] = y[i];
	...
	//---------------------------------------------------------------
{
	...
	{
		// --- take a Runge-Kutta-Cash-Karp step
		rkck_added(project,xold, n, h, derivs);
