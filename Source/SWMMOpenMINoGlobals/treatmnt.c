...
//
{
    ...
    equation = mathexpr_create_added(project, expr, getVariableIndex);
...
//  double cLost;                      // concentration lost by treatment
	...
	for (p = 0; p < project->Nobjects[POLLUT]; p++)
    {
		...
		project->Treatment = &project->Node[j].treatment[p];
		if (project->Treatment->equation == NULL) project->R[p] = 0.0;

        // --- no removal for removal-type expression when there is no inflow 
		else if (project->Treatment->treatType == REMOVAL && q <= ZERO) project->R[p] = 0.0;
	...
	else for (p = 0; p < project->Nobjects[POLLUT]; p++)
    {
		...
		if (project->Treatment->treatType == REMOVAL)
        {
...
//
{
    ...
    {
        p = varCode - PVMAX;
		if (project->Treatment->treatType == REMOVAL) return project->Cin[p];
...
//
{
    ...
    // --- apply treatment eqn.
	r = mathexpr_eval_added(project, project->Treatment->equation, getVariableValue);
    r = MAX(0.0, r);
